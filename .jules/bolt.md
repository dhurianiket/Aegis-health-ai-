...
## 2024-05-30 - [Optimize array filtering inside useMemo]
**Learning:** When filtering large arrays within React components (e.g., search/filter bars), using `(arr || []).some(callback)` and `(str || '').toLowerCase()` eagerly allocates empty arrays, inline callback functions, and temporary strings on every iteration.
**Action:** Hoist loop-invariant checks (like filter types) outside the `.filter` loop. Use early returns, check for truthiness before calling `.toLowerCase()`, and replace `.some()` with standard `for` loops to eliminate callback and array allocation overhead.
...
## 2026-09-09 - Proper Caching in Schwartzian Transform
**Learning:** When applying a Schwartzian transform to avoid O(N log N) date parsing penalties during array sorting, the cached computed values (like the parsed timestamp) must be propagated down to any subsequent  operations that also require them. Discarding the cached timestamps immediately after  only to redundantly re-parse the dates via  or  in the very next statement completely defeats the purpose of the optimization.
**Action:** When applying a Schwartzian transform (decorate-sort-undecorate), retain the cached values in the intermediate mapped objects if downstream operations (like constructing data payloads or extracting baselines) rely on them. Additionally, when using , handle its `null` return type by falling back to `NaN` (and checking with `isNaN()` in the comparator) rather than `0`, to prevent invalid dates from skewing chronological metrics.
## $(date +%Y-%m-%d) - Proper Caching in Schwartzian Transform
**Learning:** When applying a Schwartzian transform to avoid O(N log N) date parsing penalties during array sorting, the cached computed values (like the parsed timestamp) must be propagated down to any subsequent `.map()` operations that also require them. Discarding the cached timestamps immediately after `.sort()` only to redundantly re-parse the dates via `new Date()` or `parseSafeTimestamp()` in the very next statement completely defeats the purpose of the optimization.
**Action:** When applying a Schwartzian transform (decorate-sort-undecorate), retain the cached values in the intermediate mapped objects if downstream operations (like constructing data payloads or extracting baselines) rely on them. Additionally, when using `parseSafeTimestamp()`, handle its `null` return type by falling back to `NaN` (and checking with `isNaN()` in the comparator) rather than `0`, to prevent invalid dates from skewing chronological metrics.

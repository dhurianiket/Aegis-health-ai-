...
exports.PaperclipJulesCoordinator = void 0;
...
const getAiClient = () => {
    if (!aiClient) {
        const realClient = new genai_1.GoogleGenAI({ apiKey: process.env.GEMINI_API_KEY });
        const originalGenerateContent = realClient.models.generateContent.bind(realClient.models);
        realClient.models.generateContent = async function (params) {
            if (!params)
                ...
                if (isUnavailable) {
                    if (activeParams.model !== "gemini-3.6-flash") {
                        ...
                        try {
                            ...
                            const isRetryUnavailable = retryStatus === 503 ||
 ...
 */
class PaperclipJulesCoordinator {
    gcpProject;
    maxRetries = 3;
    backoffBaseMs = 2000;
    constructor() {
     ...
     */
    async orchestrateJulesFlow(config) {
     ...
     */
    async spawnSessionWithRetry(config) {
        let attempt = 0;
     ...
     */
    async pollForDeploymentPlan(sessionId) {
     ...
     */
    async validateJulesPlan(planText, originalPrompt) {
        try {
...
Respond ONLY as a structured JSON document conforming to this exact syntax boundary:
{
  ...
  "amendedSteps": ["Optional list of exact changes required if any rule is broken"]
        ...
        `,
            ...
            if (!responseText)
                throw new Error("Null response from code validator agent");
     ...
     */
    async postPlanAmendment(sessionId, feedback) {
     ...
     */
    async approveAndExecutePlan(sessionId) {
     ...
     */
    async triggerHermesFailover(taskId, triggerReason) {
     ...
     */
    async updateSessionTracker(sessionId, payload) {
...
exports.PaperclipJulesCoordinator = PaperclipJulesCoordinator;
//# sourceMappingURL=paperclipJulesCoordinator.js.map
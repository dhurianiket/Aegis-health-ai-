...
Respond ONLY as a structured JSON document conforming to this exact syntax boundary:
{
  ...
  private async triggerHermesFailover(taskId: string, triggerReason: string): Promise<void> {
    ...
    const paperclipLockRef = db.collection("users").doc(ADMIN_DEV_USER_ID).collection("systemOrchestrator").doc("locks");
    ...
    const taskLogsRef = db.collection("users").doc(ADMIN_DEV_USER_ID).collection("tasks").doc(taskId);

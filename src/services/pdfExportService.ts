...
export const exportToPDF = async (
  ...
  if (isIframe) {
    console.warn("PDF export triggered inside an iframe. If download fails, please open in a new tab.");
  }
  ...
  const rechartsContainers = clone.querySelectorAll('.recharts-responsive-container');
  ...
  try {
    ...
    if (orientation === 'landscape' || finalImgHeight <= pdfHeight) {
      // Single page fitting
      pdf.addImage(dataUrl, 'PNG', 0, 0, finalImgWidth, finalImgHeight, undefined, 'SLOW');
    } else {
      ...
      while (heightLeft > 0) {
        ...
        pdf.addImage(dataUrl, 'PNG', 0, position, finalImgWidth, finalImgHeight, undefined, 'SLOW');

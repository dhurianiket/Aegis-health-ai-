...
export const checkAppointmentsForReminders = (appointments: Appointment[]): HealthAlert[] => {
  ...
  appointments.forEach(appt => {
    ...
    if (isBefore(apptDate, endOfDay(oneDayFromNow)) && isBefore(startOfDay(oneDayFromNow), apptDate)) {
      alerts.push({
    ...
    else if (isBefore(apptDate, endOfDay(threeDaysFromNow)) && isBefore(startOfDay(threeDaysFromNow), apptDate)) {
      alerts.push({
        id: crypto.randomUUID(),
        severity: 'moderate',
        type: 'appointment',

...
export default function ProfileManagement() {
  const { profiles, activeProfile, setActiveProfile, createProfile, updateProfile, deleteProfile } = useProfile();
    ...
    <div className="space-y-6 max-w-4xl mx-auto px-4 sm:px-0">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-slate-400 text-sm mt-1">Manage multiple records and views</p>
        </div>
        {!isCreating && (
          <button 
            onClick={() => { setIsCreating(true); setCreateError(''); setNewProfileName(''); }}
      ...
      <AnimatePresence>
        {isCreating && (
          ...
          >
            <form onSubmit={handleCreate} className="bg-slate-800/50 backdrop-blur-md border border-indigo-500/30 p-6 rounded-2xl relative mb-6">
              ...
              <div className="flex justify-end mt-4">
                <button type="submit" className="px-5 py-2 bg-indigo-600 hover:bg-indigo-500 text-white rounded-xl transition-colors font-medium">
                  Create Profile
                </button>
              </div>
            </form>
              ...
              {editingId === p.id ? (
                <div>
                  ...
                  <div className="flex items-center gap-2 mt-2">
                    <button onClick={() => handleUpdate(p.id)} className="flex items-center gap-1 text-xs px-2 py-1 bg-indigo-500/20 text-indigo-300 rounded hover:bg-indigo-500/30">
                      <Save className="w-3 h-3" /> Save
                    </button>
                    <button onClick={() => setEditingId(null)} className="text-xs px-2 py-1 text-slate-400 hover:text-white">
                      Cancel
                    </button>
                  </div>
                </div>
              ) : (
                <h3 className="text-xl font-semibold text-white truncate pr-6" title={p.name}>{p.name}</h3>
              )}
            </div>
          ...
          >
            ...
            >
              ...
              <p className="text-slate-300 text-sm mb-6 leading-relaxed">
                Are you sure you want to delete this profile? This action will permanently delete all associated medical records, timeline events, and insights. This action cannot be undone.
              </p>

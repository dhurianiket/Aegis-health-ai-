      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 bg-white/5 p-8 rounded-[40px] border border-white/10">
        <div className="space-y-2">
          <div className="flex items-center gap-3">
            ...
            <h2 className="text-3xl font-bold text-white tracking-tight">Family Hub</h2>
          </div>
          <p className="text-slate-400 text-sm font-light max-w-xl">
            Aura's Multi-User interface allows you to manage dependents, share records securely, and analyze collective health risks across generations.
          </p>
        </div>
        ...
        <div className="flex bg-black/20 p-1.5 rounded-2xl border border-white/5">
          <button 
            ...
            className={`px-6 py-2.5 rounded-xl text-xs font-bold uppercase tracking-widest transition-all ${
              activeTab === 'members' ? 'bg-indigo-600 text-white shadow-xl shadow-indigo-500/20' : 'text-slate-400 hover:text-white'
            }`}
          >
            Manage Access
          </button>
          <button 
            ...
            className={`px-6 py-2.5 rounded-xl text-xs font-bold uppercase tracking-widest transition-all ${
              activeTab === 'risk' ? 'bg-indigo-600 text-white shadow-xl shadow-indigo-500/20' : 'text-slate-400 hover:text-white'
            }`}
          >
            Cross-Genetic Risk
          </button>
          ...
          <>
            ...
            <div className="space-y-6">
              <div className="bg-indigo-600 rounded-[32px] p-8 text-white shadow-2xl shadow-indigo-500/20">
                ...
                <p className="text-indigo-100 text-sm mb-6 leading-relaxed opacity-80">
                  Securely share access to your health profile with a trusted family member or physician.
                </p>
              ...
              <div className="bg-white/5 border border-white/10 rounded-[32px] p-8">
                <h4 className="text-xs font-black text-slate-500 uppercase tracking-widest mb-6">Security Invariants</h4>
                <div className="space-y-4">
                  {[
                    ...
                    <div key={i} className="flex items-center gap-3">
                      ...
                      <span className="text-xs text-slate-300 font-medium">{inv.text}</span>
                    </div>
                  ))}
                </div>
              </div>
            ...
            <div className="lg:col-span-2 space-y-6">
              <div className="flex items-center justify-between px-4">
                <h4 className="text-xs font-black text-slate-500 uppercase tracking-widest">Active Connections</h4>
              ...
              <div className="space-y-3">
                  ...
                  >
                    <div className="flex items-center gap-4">
                      <div className={`w-12 h-12 rounded-2xl flex items-center justify-center ${
                        rel.status === 'active' ? 'bg-emerald-500/10' : 'bg-amber-500/10'
                      }`}>
                        {rel.status === 'active' ? <Share2 className="w-6 h-6 text-emerald-400" /> : <Clock className="w-6 h-6 text-amber-400" />}
                      </div>
                      <div>
                        <div className="text-white font-bold text-sm tracking-tight">{rel.relatedEmail}</div>
                        <div className="flex items-center gap-2 mt-1">
                          <span className="text-[10px] text-slate-500 uppercase font-bold tracking-widest">{rel.relationType}</span>
                          <span className="w-1 h-1 rounded-full bg-slate-700" />
                          <span className={`text-[10px] font-black uppercase tracking-widest ${
                            rel.status === 'active' ? 'text-emerald-500' : 'text-amber-500'
                          }`}>{rel.status}</span>
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="flex items-center gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                      {rel.status === 'active' ? (
                        <>
                          <button className="px-4 py-2 bg-white/5 hover:bg-white/10 text-slate-300 hover:text-white rounded-xl text-[10px] font-black uppercase tracking-widest transition-all">Revoke</button>
                          <button className="px-4 py-2 bg-indigo-500/10 hover:bg-indigo-500/20 text-indigo-400 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all border border-indigo-500/20">Edit Access</button>
                        </>
                      ) : (
                        <button className="px-4 py-2 bg-white/5 hover:bg-red-500/10 text-slate-400 hover:text-red-400 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all">Cancel Request</button>
                      )}
                    </div>
                  </motion.div>
                ))}
                ...
                {relations.length === 0 && (
                  <div className="text-center py-24 bg-white/5 border border-dashed border-white/10 rounded-[40px]">
                    ...
                    <h5 className="text-slate-300 font-bold">No Active Family Links</h5>
                    <p className="text-xs text-slate-500 mt-2 max-w-sm mx-auto leading-relaxed">
                      Share access to manage chronic conditions together or compare genetic lab markers.
                    </p>
                  </div>
                )}
              </div>
            ...
            <div className="bg-white/5 border border-white/10 rounded-[40px] p-12 text-center">
              ...
              {relations.length === 0 && (
                <div className="mt-6 p-4 bg-amber-500/10 border border-amber-500/20 rounded-2xl flex items-center justify-center gap-3 max-w-md mx-auto">
                  ...
                  <span className="text-xs text-amber-200 font-medium">Link at least one family member to enable cross-analysis.</span>
                </div>
              )}
            </div>
              ...
              >
                {risks.map((risk, i) => (
                  <div key={i} className="bg-white/5 border border-white/10 rounded-[32px] p-8 flex flex-col h-full">
                    ...
                    <h5 className="text-xl font-bold text-white mb-2">{risk.condition}</h5>
                    <p className="text-xs text-slate-400 leading-relaxed mb-6 flex-grow">{risk.description}</p>
                    
                    <div className="space-y-4 mb-6">
                      <div className="flex items-center justify-between text-xs font-bold">
                        <span className="text-slate-500 uppercase tracking-widest">Hereditary Risk</span>
                        <span className="text-indigo-400">{risk.sharedRiskScore}%</span>
                      </div>
                    ...
                    <div className="space-y-2">
                       <span className="text-[10px] font-black text-slate-500 uppercase tracking-widest block mb-2">Preventive Route</span>
                       {risk.recommendations.map((rec, j) => (
                         <div key={j} className="flex items-start gap-2 text-xs text-slate-200 bg-white/5 p-2 rounded-xl border border-white/5">

...
const QuestionItem = ({ q }: { q: { question: string, reason_for_asking: string } }) => {
          ...
          >
            <div className="pt-3 border-t border-white/10">
              <p className="text-xs text-slate-400 leading-relaxed font-light">
                <strong className="text-indigo-300 font-medium">Why ask this:</strong> {q.reason_for_asking}
              </p>
            </div>
          </motion.div>
        )}
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-8">
      <div className="flex flex-col md:flex-row md:items-center gap-4 md:gap-6 mb-8 md:mb-12">
        ...
        <div>
          <h2 className="text-xl md:text-3xl font-bold tracking-tight text-white mb-1 uppercase tracking-widest">Medical Intelligence Panel</h2>
          <p className="text-slate-400 text-xs md:text-sm font-light">Cross-specialty clinical reasoning using your longitudinal digitized record.</p>
        </div>
      </div>
        ...
        <div className="xl:col-span-4 space-y-4">
              ...
              >
                <div className="relative z-10 flex items-center gap-4 md:gap-5">
                  ...
                  <div className="flex-1 min-w-0">
                    <h4 className={`font-bold uppercase tracking-tight text-xs md:text-sm transition-colors truncate ${selectedId === s.id ? 'text-white' : 'text-slate-300'}`}>{s.name}</h4>
                    <p className={`text-[9px] md:text-[10px] mt-0.5 md:mt-1 font-medium transition-colors truncate ${selectedId === s.id ? 'text-indigo-200' : 'text-slate-500'}`}>
                      {s.description}
                    </p>
                  </div>
                  <ChevronRight className={`w-4 h-4 md:w-5 md:h-5 shrink-0 transition-all ${selectedId === s.id ? 'text-white translate-x-1' : 'text-slate-700'}`} />
                </div>
                {selectedId === s.id && (
          ...
          <div className="p-6 md:p-8 bg-white/5 rounded-[32px] md:rounded-[40px] border border-white/10 mt-6 md:mt-8 hidden xl:block">
             <div className="flex items-center gap-3 mb-4 text-emerald-400">
               ...
               <span className="text-[10px] font-bold uppercase tracking-[0.2em] opacity-80">Encryption Status</span>
             </div>
             <p className="text-xs text-slate-400 font-light leading-relaxed italic">
               "Clinical synthesis occurs in an isolated memory buffer. No PHI is persisted to model training sets."
             </p>
          </div>
        </div>
            ...
            {!insight && !isAnalyzing ? (
                ...
                <div className="w-full max-w-sm mt-4 text-left">
                  <div className="bg-white/5 rounded-2xl p-4 border border-white/10 mb-6">
                    <div className="flex items-center gap-2 mb-3">
                      ...
                      <span className="text-[10px] uppercase font-bold text-slate-400 tracking-widest">Sensitivity Level</span>
                    </div>
                    <select 
                      value={sensitivity}
                    ...
                    >
                      <option value="Conservative">Conservative (Flag critical only)</option>
                      <option value="Standard">Standard (Flag out-of-range)</option>
                      <option value="High">High (Hyper-analyze all risks)</option>
                    </select>
                  </div>
                  ...
                  <div className="bg-amber-500/10 border border-amber-500/20 rounded-2xl p-4 mb-6">
                    <div className="flex items-start gap-3">
                      ...
                      />
                      <label htmlFor="disclaimer" className="text-xs text-amber-200/80 cursor-pointer leading-relaxed">
                        <strong>Medical Disclaimer:</strong> I understand that Aura is an AI assistant, not a licensed healthcare provider. Insights generated are for informational purposes only and must be verified by a board-certified physician. Do not alter medications or treatments based on this analysis.
                      </label>
                    </div>
                  </div>
                </div>
              ...
              >
                ...
                <div className="space-y-4">
                  <h3 className="text-3xl font-light text-white tracking-widest">SYNTHESIZING...</h3>
                  <div className="flex flex-col gap-2">
                    <p className="text-[10px] text-indigo-400 font-bold uppercase tracking-[0.3em]">Processing Lab Vectors</p>
                    <p className="text-[10px] text-slate-500 font-bold uppercase tracking-[0.3em] opacity-40 italic">Cross-referencing Medical Literature</p>
                  </div>
                </div>
              </motion.div>
            ) : (
              ...
              >
                  ...
                  <div className="space-y-2">
                    <div className="flex items-center gap-3">
                      <span className={`px-3 py-1 rounded-full text-[10px] font-bold uppercase tracking-widest border ${
                        insight.urgency_level === 'High' || insight.urgency_level === 'Emergency'
                ...
                <div className="grid md:grid-cols-2 gap-12">
                  <div className="space-y-6">
                    ...
                    <ul className="space-y-4">
                      {insight.observations?.map((o: any, i: number) => (
                        <li key={i} className="flex gap-4 text-sm text-slate-400 group items-start">
                  ...
                  <div className="space-y-6">
                    <h4 className="text-[10px] font-bold text-slate-500 uppercase tracking-[0.2em] flex items-center gap-3">
                      <TrendingUp className="w-4 h-4 text-indigo-400" /> Action Roadmap
                    </h4>
                    <ul className="space-y-3">
                      {insight.suggested_next_steps?.map((s: any, i: number) => (
                        <li key={i} className="flex gap-3 text-sm bg-indigo-500/5 p-4 rounded-2xl border border-indigo-500/10 text-indigo-100">
                ...
                <div className="bg-white/5 p-8 rounded-[40px] border border-white/10 space-y-6">
                  <h4 className="text-[10px] font-bold text-slate-500 uppercase tracking-[0.2em] flex items-center gap-3">
                    <AlertCircle className="w-4 h-4 text-amber-500" /> Physician Prep Questions
                  </h4>
                ...
                <div className="flex justify-center pt-8">
                   <button 
                    onClick={() => { setInsight(null); setDisclaimerAccepted(false); }}
                   ...
                   >
                     <Loader2 className="w-4 h-4 group-hover:-rotate-180 transition-transform" /> Reset & Configure Analysis
                   </button>
                </div>
              </motion.div>
            )}
          ...
          <div className="mt-auto p-4 md:p-8 border-t border-white/5 flex flex-col md:flex-row items-center justify-between bg-black/20 gap-4">
             <div className="flex items-start gap-3">
               ...
               <p className="text-[10px] text-amber-200/70 leading-relaxed max-w-2xl">
                 <strong className="text-amber-400">SAFETY WARNING:</strong> AI insights are generated purely from your provided history and literature pattern-matching. They may contain inaccuracies or hallucinated correlations. <strong>NEVER</strong> use this tool for emergency diagnosis. If experiencing acute distress, call emergency services immediately. All outputs must be clinically correlated by your physician.
               </p>
             </div>
          </div>
        </div>

...
import { getLabHistory, getDocuments, getMedications, getLatestInsights, getClinicalSummary, saveClinicalSummary } from '../../lib/firebase/firestore';
...
export default function ShareReport() {
  ...
  const handleGenerateSummary = async () => {
    ...
    try {
      ...
      const dataHashStr = pLabs.map(l => l.id).join('') + 
                          pDocs.map(d => d.id).join('') + 
                          pMeds.map(m => m.id).join('') +
                          pIns.map(i => i.id).join('') + 
                          pLabs.length + pDocs.length + pMeds.length + pIns.length;
      
      const currentDataHash = btoa(dataHashStr).slice(0, 64);

      const cachedSummary = await getClinicalSummary(user.uid, activeProfile.id);
                        ...
                        <div className="flex flex-col items-center justify-center gap-6 w-full py-4">
                          <div className="relative flex items-center justify-center">
                            ...
                            <motion.div 
                              ...
                              animate={{ boxShadow: ['0 0 15px rgba(99,102,241,0.3)', '0 0 25px rgba(99,102,241,0.6)', '0 0 15px rgba(99,102,241,0.3)'] }}
                ...
                <div className="flex flex-col h-full print:block">
                  <div className="flex items-center justify-between p-6 border-b border-white/10 bg-slate-900 print:hidden">
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        <h2 className="text-xl font-bold text-white">Clinical Summary</h2>
                        <p className="text-xs text-slate-400">Subject: {activeProfile?.name}</p>
                      </div>
                    </div>
                    ...
                    <div id="report-container" className="max-w-4xl mx-auto bg-slate-900 print:bg-white print:text-black">
                      <div className="hidden print:block mb-8 border-b border-slate-300 pb-4">
                        ...
                        <div className="mt-4 text-sm text-slate-500">
                          <p><strong>Patient:</strong> {activeProfile?.name}</p>
                      ...
                      <div className="hidden print:block mt-24 border-t border-slate-300 pt-8 text-center text-xs text-slate-400">
                        Generated securely by Aura AI Health Manager. Verification #AURA-442-99L
                      </div>
                    </div>
                  </div>
                </div>

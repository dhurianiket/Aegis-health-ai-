...
export default function ReportComparison({
  ...
  useEffect(() => {
    async function fetchData() {
      ...
      try {
        ...
        try {
          docA = await getDoc(
            doc(db, "users", user.uid, "documents", reportAId),
          );
        } catch (err) {
        ...
        try {
          docB = await getDoc(
            doc(db, "users", user.uid, "documents", reportBId),
          );
        } catch (err) {
        ...
        if (docA?.exists() && docB?.exists()) {
          const obsA =
            docA.data().extractedData?.observations ||
            docA.data().extractedData?.lab_values ||
            [];
          const obsB =
            docB.data().extractedData?.observations ||
            docB.data().extractedData?.lab_values ||
            [];
  ...
  if (errorA || errorB) {
    return (
      <div
        className="fixed inset-0 z-[100] bg-surface/95 backdrop-blur-sm overflow-y-auto w-full h-screen flex flex-col justify-center items-center"
        role="dialog"
        aria-modal="true"
      >
      ...
      <div className="bg-[var(--color-bg)] w-full max-w-5xl max-h-[90vh] rounded-2xl shadow-2xl flex flex-col border border-[var(--color-border)] overflow-hidden">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-[var(--color-border)] bg-[var(--color-surface)]">
          ...
          <button
            ...
            className="p-2 hover:bg-[var(--color-bg)] rounded-full text-[var(--color-text-muted)] transition-colors focus:outline-none focus-visible:ring-2 focus-visible:ring-current"
          >
            <X size={20} />
          </button>
        </div>
            ...
            <tbody className="divide-y divide-[var(--color-border)] text-[var(--color-text)]">
              {data?.rows.map((row, i) => {
                ...
                return (
                  <tr
                    key={i}
                    className={`hover:bg-[var(--color-surface)]/50 transition-colors ${isWorsened ? "border-l-4 border-l-amber-500 bg-amber-50/5" : "border-l-4 border-l-transparent"}`}
                  >
                    <td className="px-6 py-4 font-medium flex items-center gap-2">
                      {row.testName}
                      {row.isNewInB && (
                        <span className="px-1.5 py-0.5 rounded text-[9px] font-bold uppercase tracking-wider bg-indigo-500/10 text-indigo-500 border border-indigo-500/20">
                          NEW
                        </span>
                      )}
                    </td>
                      ...
                      {row.isNewInB ? (
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          {row.flagA && row.flagA !== "NORMAL" && (
                            <span className="text-[10px] font-bold bg-slate-500/10 text-slate-500 px-1.5 rounded">
                              {row.flagA}
                            </span>
                          )}
                        </div>
                      ...
                      {row.isMissingInB ? (
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          {row.flagB && row.flagB !== "NORMAL" && (
                            <span
                              className={`text-[10px] font-bold px-1.5 rounded ${isWorsened ? "bg-amber-500/10 text-amber-600" : "bg-slate-500/10 text-slate-500"}`}
                            >
                              {row.flagB}
                            </span>
                          )}
                    ...
                    <td className="px-6 py-4 font-mono text-xs">
                      {row.deltaPercent !== null ? (
                        <div
                          className={`flex items-center gap-1 ${isWorsened ? "text-amber-500" : isImproved ? "text-green-500" : "text-slate-500"}`}
                        >
                          {row.direction === "worsened" ? (
                            <ArrowUpRight
                              size={14}
                              className={row.delta! < 0 ? "rotate-90" : ""}
                            />
                        ...
                        <span className="text-slate-500">
                          {row.direction === "unchanged" ? "Unchanged" : "-"}
                        </span>
                      )}
                    </td>
                  </tr>
              ...
              {data?.rows.length === 0 && (
                <tr>
                  <td colSpan={5} className="px-6 py-12 text-center text-muted">
                    No comparable data points found between these reports.
                  </td>
                </tr>
              )}
            </tbody>

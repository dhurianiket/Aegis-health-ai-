    ...
    for (let i = 0; i < topMarkers.length; i++) {
      ...
      for (let j = 0; j < topMarkers.length; j++) {
        if (i === j) {
          ...
          dates1.forEach(date => {
            if (byMarkerAndDate[m2] && byMarkerAndDate[m2][date] !== undefined) {
  ...
  return (
    <div className="bg-white/5 backdrop-blur-xl border border-white/10 p-6 rounded-[32px] shadow-2xl">
      <div className="flex items-center gap-3 mb-6">
        ...
        <div>
          <h3 className="text-lg font-bold text-white tracking-tight">Biomarker Correlation</h3>
          <p className="text-xs text-slate-400 mt-0.5">Statistical relationships between labs</p>
        </div>
      </div>
      ...
      <div className="overflow-x-auto">
        <div className="min-w-max">
            <div className="flex mb-2">
                ...
                {markers.map(m => (
                    <div key={m} className="w-16 text-center text-[10px] font-bold text-slate-400 uppercase truncate px-1">
                        {m}
                    </div>
                ))}
            </div>
            ...
            {markers.map((m1, i) => (
                <div key={m1} className="flex mb-2 items-center">
                    ...
                    {markers.map((m2, j) => (
                        <div key={`${m1}-${m2}`} className="w-16 flex justify-center px-1">
                            <motion.div 
      ...
      <div className="mt-4 flex items-start gap-2 bg-slate-900/40 p-3 rounded-xl border border-white/5">
          ...
          <p className="text-xs text-slate-400 leading-relaxed">
            Values near <span className="text-indigo-400 font-bold">1.0</span> indicate markers rise together. Values near <span className="text-amber-400 font-bold">-1.0</span> indicate an inverse relationship.
          </p>
      </div>
    </div>
  );

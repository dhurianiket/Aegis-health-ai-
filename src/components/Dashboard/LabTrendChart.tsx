    ...
    if (active && payload && payload.length) {
      ...
      return (
        <div className="bg-white/90 dark:bg-black/80 backdrop-blur-md border border-[var(--color-border)] p-4 rounded-2xl shadow-xl min-w-[150px]">
          <p className="text-slate-600 dark:text-white/70 text-xs font-bold uppercase tracking-widest mb-2">
            {data.date}
          </p>
          ...
          {data.referenceRange && (
            <p className="text-slate-600 dark:text-white/70 text-xs mt-1">
           ...
           <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3">
             <div className="relative">
               <select
                 ...
                 className="appearance-none bg-[var(--color-surface)] border border-[var(--color-border)] text-[var(--color-text)] text-xs font-medium tracking-widest rounded-xl px-4 py-2 pr-8 focus:outline-none focus-visible:ring-2 focus:ring-[var(--color-primary)] cursor-pointer shadow-sm transition-colors"
               >
                 {uniqueMarkers.map((m) => (

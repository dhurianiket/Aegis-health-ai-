...
export const OrganHealthAvatar: React.FC<OrganHealthAvatarProps> = ({ labObservations = [] }) => {
    ...
    if (status === 'critical') {
      return (
        <span className="px-2.5 py-0.5 rounded-full bg-rose-500/20 text-rose-300 border border-rose-500/40 text-[10px] font-bold flex items-center gap-1">
          <AlertTriangle className="w-3 h-3" /> Critical Shift
        </span>
      );
    }
    if (status === 'warning') {
      return (
        <span className="px-2.5 py-0.5 rounded-full bg-amber-500/20 text-amber-300 border border-amber-500/40 text-[10px] font-bold flex items-center gap-1">
          <Activity className="w-3 h-3" /> Warning Drift
        </span>
      );
    }
    return (
  ...
  const getScoreColor = (score: number) => {
    if (score < 65) return 'text-rose-400 border-rose-500/50 shadow-[0_0_20px_rgba(244,63,94,0.3)]';
    if (score < 85) return 'text-amber-400 border-amber-500/50 shadow-[0_0_20px_rgba(245,158,11,0.3)]';
    return 'text-emerald-400 border-emerald-500/50 shadow-[0_0_20px_rgba(16,185,129,0.3)]';
  };

  return (
    <div className="bg-slate-900/90 dark:bg-slate-900/95 backdrop-blur-2xl border border-emerald-500/30 shadow-[0_16px_40px_-8px_rgba(16,185,129,0.2)] rounded-[32px] p-6 md:p-8 space-y-6">
      {/* Header */}
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <div className="flex items-center gap-2">
              <h3 className="text-xl font-bold text-white tracking-wide">3D Organ System Health Avatar</h3>
              <span className="px-2.5 py-0.5 rounded-full bg-emerald-500/10 border border-emerald-500/30 text-emerald-300 font-mono text-[10px] font-semibold">
                6 Organ Systems
              </span>
            </div>
            <p className="text-xs text-slate-300 font-light mt-0.5">
              Anatomical physiological risk mapping driven by active biomarker panels
            </p>
          </div>
        </div>

        {/* Overall Score Pill */}
        <div className="flex items-center gap-3 bg-slate-950/80 px-4 py-2 rounded-2xl border border-white/10">
          <div className="text-right">
            <div className="text-[10px] text-slate-400 uppercase tracking-wider font-semibold">Overall Physiology</div>
            <div className="text-xs text-emerald-400 font-bold">
              {overview.optimalCount} Optimal · {overview.warningCount} Warning · {overview.criticalCount} Critical
            </div>
          </div>
          <div
            className={`w-12 h-12 rounded-full border-2 flex items-center justify-center font-black text-lg ${getScoreColor(
        ...
        {Object.values(overview.organSystems).map((organ) => {
          ...
          return (
            <div
              ...
              className="bg-slate-950/80 hover:bg-slate-950 border border-white/10 hover:border-emerald-500/40 rounded-2xl p-4 transition-all duration-300 cursor-pointer group flex flex-col justify-between space-y-4 shadow-md"
            >
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                  <div className={`p-2.5 rounded-xl bg-white/5 border ${scoreClass}`}>
                ...
                <div
                  className={`w-10 h-10 rounded-full border flex items-center justify-center font-bold text-xs ${scoreClass}`}
                >
                  {organ.score}
                </div>
              </div>
              ...
              <div className="pt-2 border-t border-white/5 flex items-center justify-between text-[11px] text-emerald-400 font-semibold group-hover:translate-x-1 transition-transform">
      ...
      {selectedOrgan && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-md animate-fade-in">
          <div className="bg-gradient-to-b from-[#0F2647] via-[#0A192F] to-[#071325] border border-emerald-500/30 rounded-3xl p-6 md:p-8 max-w-lg w-full shadow-2xl space-y-6 relative overflow-hidden">
            <div className="flex items-center justify-between border-b border-white/10 pb-4">
              <div className="flex items-center gap-3">
                <div className={`p-3 rounded-2xl bg-white/5 border ${getScoreColor(selectedOrgan.score)}`}>
                  {React.createElement(getOrganIcon(selectedOrgan.key), { className: 'w-6 h-6' })}
                </div>
                <div>
                  <h3 className="text-xl font-bold text-white">{selectedOrgan.displayName}</h3>

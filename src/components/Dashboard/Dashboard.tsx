...
const aggregateLabs = (docs: MedicalDocument[]): any[] => {
  ...
  docs.forEach((doc) => {
    ...
    const allExtractedLabs = [
      ...(Array.isArray(labValues) ? labValues : []),
      ...
      const entry = {
        ...
        date:
          ...
          (typeof doc.createdAt === "string"
            ? doc.createdAt
            : (doc.createdAt as any)?.toDate?.()?.toISOString()) ||
...
export default function Dashboard({
  ...
  useEffect(() => {
    ...
    async function fetchData() {
      ...
      try {
        ...
        const [scoresResult, insightsResult, documentsResult] =
        ...
        if (isMounted) {
          ...
          setError(
            docsError ? "Failed to load this report. Tap to retry." : null,
          );
        }
      } catch (err) {
        ...
        if (isMounted) {
          setError(
            "Failed to load health telemetry. Please check your connection.",
          );
        }
      } finally {
        if (isMounted) {
  ...
  const abnormalLabs = useMemo(() => {
    return keyLabs.filter((l) => {
      const status = (l.status as string)?.toLowerCase();
      return (
        status === "high" ||
        status === "abnormal" ||
        status === "low" ||
  ...
  const importantMarkers = useMemo(() => {
    ...
    return keyLabs.filter((l) =>
      importantNames.has(l.markerName.toLowerCase().trim()),
    );
  }, [keyLabs]);
  ...
  return (
    ...
    >
      ...
      {loading && !error && keyLabs.length === 0 && <DashboardSkeleton />}
      ...
      {!loading &&
        ...
        (keyLabs.length > 0 ||
          ...
          <div className="flex flex-col lg:flex-none">
            <motion.div
              variants={tileVariants}
              className="order-first lg:order-none w-full mb-8 space-y-6"
            >
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="space-y-6">
                  {user && <RemindersWidget userId={user.uid} />}
                  {activeProfile?.reproductiveProfile?.cycleTrackingEnabled &&
                    activeProfile?.reproductiveProfile?.menstruates && (
                      <Suspense
                        fallback={
                          <div className="h-40 w-full animate-pulse bg-surface/50 rounded-3xl" />
                        }
                      >
            ...
            >
              ...
              <div className="lg:col-span-1 bg-[var(--color-surface)] backdrop-blur-xl border border-[var(--color-border)] p-6 rounded-[32px] shadow-md dark:shadow-2xl">
                ...
                <div className="h-[300px] w-full">
                  <Suspense
                    fallback={
                      <div className="h-[300px] w-full animate-pulse bg-surface/50 rounded-3xl flex items-center justify-center text-muted">
                        Loading chart...
                      </div>
                    }
                  >
                ...
                <div className="space-y-4">
                  {abnormalLabs.slice(0, 5).map((lab, i) => (
                    <div
                      ...
                      className="flex flex-col p-4 bg-[var(--color-bg)] hover:bg-[var(--color-surface)] rounded-2xl border border-[var(--color-border)] transition-all cursor-pointer group"
                    >
                      <div className="flex justify-between items-center mb-2">
                        ...
                        <span
                          className={`px-2 py-0.5 rounded text-[10px] font-bold uppercase tracking-wider ${(lab.status as any) === "low" ? "bg-orange-500/10 text-orange-500" : "bg-red-500/10 text-red-500"}`}
                        >
                          {lab.status}
                        </span>
                      </div>
                  ...
                  {abnormalLabs.length === 0 && (
                    <p className="text-sm text-muted">
                      All tracked markers are within normal ranges.
                    </p>
                  )}
                </div>
              </div>
              ...
              <div className="bg-[var(--color-surface)] backdrop-blur-xl p-8 rounded-[40px] border border-[var(--color-border)] shadow-md dark:shadow-2xl">
                ...
                <div className="grid grid-cols-2 gap-4">
                  {importantMarkers.map((lab, i) => {
                    ...
                    if (
                      (lab as any).history &&
                      (lab as any).history.length > 1
                    ) {
                      const prev = (lab as any).history[1];
                      const prevValRaw = parseFloat(
                        String(prev.value || prev.display_value).replace(
                          /[^0-9.-]/g,
                          "",
                        ),
                      );
                      prevValNum = isNaN(prevValRaw)
                        ? numericValue
                        : prevValRaw;
                      const d = parseSafeTimestamp(prev.date);
                      if (d && !isNaN(d.getTime())) {
                        prevDateStr = d.toLocaleDateString("en-US", {
                          month: "short",
                          day: "numeric",
                        });
                      }
                    ...
                    if (lab.referenceRange) {
                      const rangeMatch = String(lab.referenceRange).match(
                        /([0-9.]+)\s*-\s*([0-9.]+)/,
                      );
                      if (rangeMatch) {
                        ...
                        const lessMatch = String(lab.referenceRange).match(
                          /<\s*([0-9.]+)/,
                        );
                        if (lessMatch) {
                          refHigh = parseFloat(lessMatch[1]);
                        }
                        const greaterMatch = String(lab.referenceRange).match(
                          />\s*([0-9.]+)/,
                        );
                        if (greaterMatch) {
                    ...
                    return (
                      <div
                        key={i}
                        className="rounded-2xl border border-surface bg-surface/50 overflow-hidden [&_>_div]:p-4"
                      >
                        <HeroMetric

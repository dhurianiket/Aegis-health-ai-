  ...
  return (
    <div className="bg-white/5 backdrop-blur-xl border border-white/10 p-6 rounded-[32px] shadow-2xl">
      <div className="flex items-center gap-3 mb-6">
        ...
        <div>
          <h3 className="text-lg font-bold text-white tracking-tight">Key Biomarkers</h3>
          <p className="text-xs text-slate-400 mt-0.5">Recent trajectory overview</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {sparklinesData.map((item, idx) => {
          ...
          const isDown = item.previous && item.latest.value < item.previous.value;
          const delta = item.previous ? item.latest.value - item.previous.value : 0;
          const isCritical = item.latest.status === 'critical' || item.latest.status === 'abnormal';
          ...
          return (
            ...
            >
              <h4 className="text-[10px] font-bold text-slate-400 uppercase tracking-widest truncate">{item.name}</h4>
              <div className="flex items-end justify-between mt-2">
                <span className={`text-xl font-light tracking-tight ${isCritical ? 'text-amber-400' : 'text-white'}`}>
                  {item.latest.value}
                </span>
                {item.previous && (
                  <div className={`flex items-center text-[10px] font-bold ${isUp ? 'text-red-400' : isDown ? 'text-emerald-400' : 'text-slate-500'}`}>

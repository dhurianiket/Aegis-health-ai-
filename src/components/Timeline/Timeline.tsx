  ...
  return (
    <div className="max-w-4xl mx-auto space-y-12 pb-20">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h2 className="text-2xl md:text-3xl font-bold tracking-tight text-white mb-2">Clinical Narrative</h2>
          <p className="text-slate-400 text-xs md:text-sm font-light">A longitudinal reconstruction of every clinical interaction for {activeProfile?.name || 'this profile'}.</p>
        </div>
        ...
        <div className="space-y-8 md:space-y-12">
          <AnimatePresence mode="popLayout">
            {isLoading ? (
                ...
                return (
                  ...
                  >
                    <div className="absolute left-0 top-0 w-12 h-12 md:w-16 md:h-16 rounded-2xl md:rounded-3xl bg-[#0F172A] border border-white/10 flex items-center justify-center shadow-2xl z-10 transition-transform group-hover:scale-110">
                      <config.icon className={`w-5 h-5 md:w-7 md:h-7 text-${config.color}-400`} />
                    </div>

                    <div onClick={() => setSelectedDoc(doc)} className="bg-white/5 backdrop-blur-xl p-6 md:p-8 rounded-3xl md:rounded-[40px] border border-white/10 shadow-3xl hover:bg-white/10 transition-all cursor-pointer group/card flex-1 ml-4 md:ml-8">
                      <div className="flex flex-col md:flex-row md:items-start justify-between gap-4 md:gap-6">
                        <div className="space-y-2 md:space-y-3 flex-1">
                          <p className="text-[9px] md:text-[10px] font-bold text-slate-500 uppercase tracking-[0.2em] mb-1">
                            {new Date(doc.date).toLocaleDateString('en-US', { month: 'short', day: '2-digit', year: 'numeric' })}
                          </p>
                          ...
                          <div className="flex items-center gap-2">
                             <span className="text-[10px] md:text-xs font-semibold text-indigo-400 italic opacity-80">{doc.hospitalName || 'Independent Provider'}</span>
                          </div>
                          <p className="text-slate-400 text-xs md:text-sm leading-relaxed font-light mt-2 md:mt-3 truncate max-w-xl">
                            Record for {doc.fileName} — {doc.doctorName ? `Physician: ${doc.doctorName}` : 'Self-uploaded'}
                          </p>
                        </div>
                      </div>
                      ...
                      <div className="mt-6 md:mt-8 pt-4 md:pt-6 border-t border-white/5 flex flex-col md:flex-row md:items-center justify-between gap-4">
                        ...
                        <div className="md:ml-auto flex items-center gap-2 text-indigo-400 font-bold text-[9px] md:text-[10px] uppercase tracking-widest group-hover/card:underline">
                          Digitized Record <ArrowUpRight className="w-3 h-3 md:w-4 md:h-4" />
                        </div>
                      </div>
                    </div>
                  </motion.div>
              ...
              <motion.div layout initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="text-center py-20 bg-white/5 rounded-[40px] border border-dashed border-white/10">
                <FileText className="w-12 h-12 text-slate-700 mx-auto mb-4" />
                <p className="text-slate-500 font-medium text-sm">No records found for this category.</p>
              </motion.div>
            )}
          </AnimatePresence>
        </div>
      ...
      <div className="p-6 md:p-10 bg-indigo-600 rounded-3xl md:rounded-[48px] shadow-2xl shadow-indigo-500/20 border border-white/20 flex flex-col md:flex-row gap-6 md:gap-8 items-start relative overflow-hidden">
        ...
        <div className="relative z-10">
          <h4 className="font-bold text-white text-base md:text-lg uppercase tracking-tight mb-2">Continuity Gap Detected</h4>
          <p className="text-indigo-100 text-xs md:text-sm leading-relaxed font-light italic">
            "Your longitudinal record shows a lack of recent checkups. Synchronizing historical records or scheduling a screening is recommended for profile completeness."
          </p>
        </div>
            ...
            >
              <div className="flex items-center justify-between p-6 md:p-8 border-b border-white/5 relative shrink-0">
                <div>
                   ...
                   <p className="text-slate-400 text-sm mt-1">{selectedDoc.date} • {selectedDoc.hospitalName || 'Unknown Provider'}</p>
                </div>
                <button 
              ...
              <div className="p-6 md:p-8 overflow-y-auto flex-1 space-y-8">
                {selectedDoc.extractedData?.findings ? (
                  <div className="space-y-3">
                    <h4 className="text-xs font-bold text-slate-500 uppercase tracking-widest">Clinical Findings</h4>
                  ...
                  <p className="text-slate-500 text-sm italic">No extracted findings available.</p>
                )}

                {selectedDoc.extractedData?.lab_values && selectedDoc.extractedData.lab_values.length > 0 && (
                  <div className="space-y-4">
                    <h4 className="text-xs font-bold text-slate-500 uppercase tracking-widest">Lab Results</h4>
                    <div className="grid gap-3">
                      {selectedDoc.extractedData.lab_values.map((lab: any, i: number) => {
                        const isCritical = lab.status === 'critical' || lab.status === 'abnormal';
                        return (
                          <div key={i} className={`p-4 rounded-2xl border flex items-center justify-between ${isCritical ? 'bg-red-500/5 border-red-500/20' : 'bg-white/5 border-white/10'}`}>
                            <div>
                               <p className="font-bold text-slate-200 text-sm">{lab.marker}</p>
                               <p className="text-[10px] text-slate-500 font-bold uppercase mt-1">Ref: {lab.reference_range || '-'}</p>
                            </div>
                            <div className="text-right">
                               <p className={`text-lg font-light ${lab.status === 'abnormal' ? 'text-amber-400' : lab.status === 'critical' ? 'text-red-400' : 'text-emerald-400'}`}>
                                 {lab.value} <span className="text-xs opacity-50">{lab.unit}</span>
                               </p>
                            </div>
                          </div>
                        );
                ...
                {selectedDoc.extractedData?.medications && selectedDoc.extractedData.medications.length > 0 && (
                  <div className="space-y-4">
                    <h4 className="text-xs font-bold text-slate-500 uppercase tracking-widest">Medications</h4>
                    <div className="grid gap-3">
                      {selectedDoc.extractedData.medications.map((med: any, i: number) => (
                          <div key={i} className="p-4 rounded-2xl border border-white/10 bg-white/5 flex items-center justify-between">
                            <div>
                               <p className="font-bold text-slate-200 text-sm">{med.name}</p>
              ...
              <div className="p-6 md:p-8 border-t border-white/5 flex items-center gap-3 shrink-0 flex-wrap">
                 <button onClick={() => handleDownload(selectedDoc)} className="flex items-center gap-2 px-4 py-2.5 bg-indigo-600 hover:bg-indigo-500 text-white rounded-xl text-xs font-bold uppercase tracking-widest transition-colors flex-1 justify-center">
                   <Download className="w-4 h-4" /> Download JSON
                 </button>
                 <button onClick={() => handleShare(selectedDoc)} className="flex items-center gap-2 px-4 py-2.5 bg-white/10 hover:bg-white/20 text-white rounded-xl text-xs font-bold uppercase tracking-widest transition-colors flex-1 justify-center">
                   <Share2 className="w-4 h-4" /> Share Info
                 </button>
                 <button onClick={() => handleDelete(selectedDoc)} disabled={isDeleting} className="flex items-center gap-2 px-4 py-2.5 bg-red-500/10 hover:bg-red-500/20 text-red-500 rounded-xl text-xs font-bold uppercase tracking-widest transition-colors flex-1 justify-center disabled:opacity-50">

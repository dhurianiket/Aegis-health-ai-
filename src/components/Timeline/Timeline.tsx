...
export default function Timeline() {
  ...
  const sortedTrends = useMemo(() => {
    return Object.values(trends).sort((a, b) => {
      return (
        (SEVERITY_MAP[a.latestFlag || "NORMAL"] ?? 4) -
        (SEVERITY_MAP[b.latestFlag || "NORMAL"] ?? 4)
      );
    });
  }, [trends]);
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-12 pb-20">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <div className="flex bg-slate-100 dark:bg-slate-800/50 p-1.5 rounded-2xl border border-slate-200 dark:border-white/5 overflow-x-auto w-full md:w-auto scrollbar-none snap-x scroll-mask-right">
          {categories.map((cat) => (
            <button
              key={cat.id}
              onClick={() => setFilterType(cat.id)}
              aria-label={cat.label}
              className={`px-4 py-2 rounded-xl text-xs font-bold uppercase tracking-widest whitespace-nowrap transition-all duration-100 ease-out active:scale-95 snap-start cursor-pointer ${
                filterType === cat.id
